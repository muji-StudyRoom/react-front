      ...
      if (props.target === "all") {
        ...
        for (let i = response.data.length - 1; i >= 0; i--) {
          ...
          let displayCreate =
            ...
            "일 " +
            createAt.substring(11, 13) +
            "시 " +
            createAt.substring(14, 16) +
            "분";
          roomList.push([
            <div className="room" key={response.data[i].roomId}>
              <div
                ...
                onClick={openModal}
                id={response.data[i].roomId}
                style={{
                  backgroundImage: `url(/box/kakao${(parseInt(i) % 10) + 1}.jpg)`,
                }}
              ></div>
              <div className="room-body">{response.data[i].roomName}</div>
              <div className="room-footer">
                <div>
                  <FontAwesomeIcon icon={faUser} /> {response.data[i].roomEnterUser} / {response.data[i].roomCapacity}
                  {" 명"}
                </div>
                <div>
        ...
        for (let i = props.searchData.length - 1; i >= 0; i--) {
          ...
          roomList.push([
            <div className="room" key={props.searchData[i].roomId}>
              ...
              <div className="room-footer">
                <div>
                  <FontAwesomeIcon icon={faUser} /> {props.searchData[i].roomEnterUser} / {props.searchData[i].roomCapacity}
                  {" 명"}
                </div>
                <div>
                  <FontAwesomeIcon icon={faCalendarDays} /> {displayCreate}
